...
```bash
# 1) 전체 채점 + 컨택트 시트
python3 -m idphoto catalog "<후보>/*.jpg" --refs "<원본>/*.jpg" --out out/판정

# 2) 남은 후보의 골격을 눈으로 확인 (§5 — 자동으로 못 잡는 구간)
python3 -m idphoto overlay "<후보>/*.jpg" --refs "<원본1장>.jpg" --out out/판정/overlay
```

1. `contact_sheet.png` 로 전체를 훑는다 — 테두리 색으로 1차 분류
2. 하드 게이트 탈락분은 그대로 버린다 (§1-1)
3. 통과분에서 등급 기준(§2 또는 §3)의 목표치를 넘는 것만 남긴다
4. **`overlay_*.png` 의 3번 패널에서 턱선·볼 라인만 본다.**
   초록(원본 윤곽)이 빨강(후보 윤곽) **바깥**에 있으면 얼굴이 깎인 것이다.
   실측 확인: 12% 깎기가 이 방식으로 눈에 보인다 — 자동 검사는 놓치는 구간이다
5. 애매하면 탈락
...
## 7. 기준을 사람 판단에 맞춰 되돌리기

이 문서의 수치는 **초기값**이다. 표본이 1인이고, 자동 검사가 못 잡는 구간이
있음을 §5 에서 인정했다. 그래서 사람이 본 결과로 기준을 되맞추는 절차를 둔다.

```bash
# 1) catalog 이 낸 scores.csv 의 human 열에 pass/fail 을 채운다
# 2) 자동 판정과 어디서 갈리는지, 각 지표를 어디서 끊어야 맞는지 본다
python3 -m idphoto calibrate out/판정/scores.csv
```

출력이 알려주는 것:

- **일치율** — 자동 판정이 사람 판단을 얼마나 재현하는가
- **불일치 목록** — 기준을 고칠 근거가 되는 사진들. 두 방향을 구분해서 보여준다
  (자동은 통과시켰는데 사람이 탈락 / 그 반대)
- **지표별 최적 임계값과 균형정확도** — 사람 판단을 가장 잘 가르는 지점

**균형정확도 80% 미만인 지표는 사람 판단을 설명하지 못한다.** 그 축은 기준에서
빼거나, 사람이 실제로 보고 있는 다른 것을 찾아야 한다. 임계값을 감으로
올리고 내리는 대신 이 숫자를 보고 움직인다.

라벨이 쌓일수록 기준이 정확해진다. **한 번에 완성하려 하지 말고, 판정할 때마다
human 열을 채워 두는 것이 이 절차의 전부다.**
