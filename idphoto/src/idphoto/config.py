...
class QAConfig:
    ...
    # 본인 같음. 2026-09-13 실사진 실측으로 확정:
    #
    #   OpenCV SFace 동일인 판정 레퍼런스        0.363
    #   같은 사람의 서로 다른 실사진 2장(최악)    0.547   ← 자연 하한
    #   인핸즈 출력 vs 참조 평균 임베딩           0.646 ~ 0.674
    #   인핸즈 출력 2장끼리                      0.946
    #
    # 같은 사람을 다른 날 찍어도 0.547 밖에 안 나온다. 생성물에 그보다 높은
    # 값을 요구하는 것은 무의미하므로 하드 게이트는 그 아래에 둔다.
    # 목표치는 인핸즈 수준(0.65)으로 잡는다 — 경쟁 기준선이다.
    tau_id: float = 0.50
    tau_id_target: float = 0.65
    ...
    # 피부색 이동 상한.
    #
    # 처음에는 L*(밝기) 로 잡았는데, 실측해 보니 그건 틀린 지표였다.
    # 인핸즈 출력은 입력 대비 ΔL* +29 다 — 실내 셀카를 스튜디오 조명으로
    # 다시 비추면 당연한 일이고, 사용자도 그 결과를 만족스럽게 쓰고 있다.
    # L* 로 게이트를 걸면 정상적인 생성물이 전부 탈락한다.
    #
    # 같은 측정에서 채도 좌표는 거의 그대로였다: Δa* +0.0, Δb* +3.0.
    # 밝기는 조명이 정하고, **색조는 사람이 정한다**. 톤 밝히기 편향을
    # 잡으려면 a*/b* 를 봐야 한다.
    max_skin_chroma_shift: float = 9.0   # sqrt(Δa*² + Δb*²)
